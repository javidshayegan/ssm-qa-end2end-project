...
Feature: As OR Scheduler, I want to request revision
  ...
  Scenario: filling out patient details
    ...
    Then "user" fills the form with the following information in form
      | FIRST NAME | LAST NAME  | MIDDLE NAME   | DATE OF BIRTH | SSN        | PHONE NUMBER   | EMAIL          | LANGUAGE | SEX | LATEX ALLERGY |
      | Brad		 | Pitts      | Test	  |01/01/1990    | 093-23-3914 | (314)-323-3223 | ajkdh@email.com    | Spanish | M           | NO            |
  ...
  Scenario: Adding a Procedure
    ...
    And "user" adds surgical procedures information
      | SURGEON       | CPT CODE(S) | PROCEDURE NAME       | MODIFIER SIDE | MODIFIER APPROACH | PRIMARY PROCEDURE? | DURATION | ANESTHESIA TYPE | IMPLANTS NEEDED | VENDOR | VENDOR CONTACTED |OTHER EQUIPMENT AND SUPPLIES | ADDITIONAL PROCEDURE COMMENTS |
      | David Moore | 23410       | Shoulder Arthroscopy | Left          | Anterior          | 	YES                | 60       | General         | YES               | Vendor Test | YES        |None                         | None                          |
  ...
  Scenario: filling pre-admit testing details
    ...
    Then "user" fills the form with the pre-admit form with the following information
      | IS A PRE-HOSPITAL BED NEEDED? | IS PRE-ADMISSION TESTING NEEDED | WHERE IS PAT TAKING PLACE |
      | Direct Admit                  | YES                              | PAT                      |
    And "user" clicks "Submit Surgery Request"
    And Browser quits

  Scenario: Request revision against procedure
    ...
    | TIME_NOT_AVAILABLE | MISSING_INFORMATION | OTHER | MESSAGE |
    | V                  | V                   |       | Request Revision Test |
    Then the request has Needs Revision alert
    | Time not available | Missing information | Request Revision Test |
    And Browser quits
