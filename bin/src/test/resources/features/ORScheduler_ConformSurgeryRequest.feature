@ORScheduler_ConformSurgeryRequest
Feature: As OR Scheduler, I want to conform a request surgery
  ...
  Scenario: going back to the homepage and verify the request
    ...
    Then "user" should be able to see the request just made
      ...
      | Brad, Pitts Test | Shoulder Arthroscopy | St. Thomas Midtown BH CSC | Cordovez MD, Leah   |
    And Browser quits
  Scenario: Conforming a request surgery
  When OR Scheduler is on case tracker landing page
  And OR Scheduler sets unscheduled request to being scheduled
