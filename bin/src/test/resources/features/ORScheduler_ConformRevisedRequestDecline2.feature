@ORScheduler_ConformRevisedRequestDecline2
Feature: OR Scheduler, I want to decline request revision initiated by Office Scheduler - 2
...
@Approve0
  ...
  Scenario: OR Scheduler declines request revision initiated by office scheduler
    ...
    When OR Scheduler declines the request revision initiated by office scheduler
      ...
      | V                  |                    | V      | Request Revision - 1 Test |
    Then Alerts have NEEDS REVISION
      | Time not available | Other | Request Revision - 1 Test |
    When OR Scheduler declines the request revision initiated by office scheduler
      | TIME_NOT_AVAILABLE | MISSING_INFORMATION | OTHER | MESSAGE |
      | V                  | V                   |       | Request Revision - 2 Test |
    Then Alerts have NEEDS REVISION
      | Time not available | Missing information | Request Revision - 2 Test |
