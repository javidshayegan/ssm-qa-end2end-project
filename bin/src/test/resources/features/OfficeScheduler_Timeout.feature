@OfficeScheduler_Timeout
Feature: As Office Scheduler, I validate that a user can't Edit a procedure after Five-minutes of being Inactive
 or user logs out after 15-minutes of being Inactive 
...
@Test
	Scenario: filling out patient details
		...
		| FIRST NAME | LAST NAME  | MIDDLE NAME   | DATE OF BIRTH | SSN        | SEX | PHONE NUMBER   | EMAIL          | LANGUAGE| LATEX ALLERGY |
		| Brad		 | Pitts      | Test	  |01/01/1990    | 093-23-3914 | F              |(314)-323-3223 | ajkdh@email.com    | Spanish| YES         |
...
@Test
	Scenario: going back to the homepage and verify the request
		...
		| Brad, Pitts Test | Shoulder Arthroscopy | Ascension Saint Thomas Hospital Midtown BH CSC | Moore MD, David |

@Test1
	Scenario:  Verify user can not Edit a procedure after Five-minutes of being Inactive
		...
		Given "user" fills the form with the following information in the form
		|Date	|
		|		|
		And "user" verifies the following warning is displayed

@Test1		
	Scenario: validate user logs out after Fifteen minutes of being Inactive, make sure to not make any mouse or keyboard movement during this session
		Given "user" is on "https://casetracker-qa.app.cloud-02.pcf.ascension.org/?page=1&size=25&sortModel=procedureDate:asc"
		And "user" waits Sixteen minutes with no mouse or keyboard movement
		When "user" clicks on Request a Surgery button again
		Then "user" is taken to logout page or user logs out
		And Browser quits
