@All_RequestCancellation
Feature: As Office Scheduler, I want to cancel a surgery request
  
  Scenario: validate request surgery page contents
   Given "user" is on "https://casetracker-qa.app.cloud-02.pcf.ascension.org"
  ...
  Scenario: filling out appointment details
    ...
    Then "user" fills the form with the following information
      ...
      | Ascension Saint Thomas Hospital Midtown BH CSC    |            |       | A.M.     | Check	   | Check          | Inpatient                |
  ...
  Scenario: filling out patient details
   Given "user" is on the Patient Details form
		...
		|FIRST NAME |LAST NAME  |MIDDLE NAME	|DATE OF BIRTH |SEX	|SSN		 | PHONE NUMBER   | EMAIL          |LANGUAGE	|LATEX ALLERGY	|IODINE ALLERGY |
		|Brad		|Pitts      |Test			|01/01/1990    |M	|093-23-3914 | (314)-323-3223 | ajkdh@email.com|Spanish		|YES        	|YES            |
  ...
  Scenario: Adding a Procedure
   ...
   And "user" adds surgical procedures information
 	...
 	| Werther DMD, John R | 23410       | Shoulder Arthroscopy | Left          | Anterior          | 	YES                | 60       | General         | NO               | None                         | None                          |
  ...
  Scenario: going back to the homepage and verify the request
   ...
   Then "user" should be able to see the request just made
	...
	| Brad, Pitts Test | Shoulder Arthroscopy | Ascension Saint Thomas Hospital Midtown BH CSC| Werther DMD, John R |
	And Unscheduled in task area should increment by one

  Scenario: Cancel the surgery procedure
  ...
  Scenario: OR Scheduler confirms pending request
    When OR Scheduler confirms pending cancellation
    Then Case card has Cancellation request approved alert
    And Browser quits

  Scenario: Office Scheduler sees approval for cancellation
