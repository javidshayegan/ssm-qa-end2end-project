@ORScheduler_ConformRevisedRequestDecline1
Feature: OR Scheduler, I want to decline request revision initiated by Office Scheduler_1
...
@Approve0
  ...
  Scenario: Request revision against procedure
    ...
    When OR Scheduler makes request revision
      ...
      |                   |          V          |  V     | please update vendor name |
    Then the request has Needs Revision alert
      | Missing information | Other | please update vendor name |

  Scenario: Office Scheduler mark as resolved
    Given Office Scheduler is on case tracker landing page
    When Office Scheduler marks as resolved
      | CHANGE_PROCEDURE_TIME | UPDATE_INFORMATION | OTHER | MESSAGE |
      |                      |          V          |       | Mark As Resolved Test |
    Then the request has Revised alert
      | Updated Information | Mark As Resolved Test |
    And Browser quits

  Scenario: OR Scheduler declines request revision initiated by office scheduler
    Given OR Scheduler is logged on
    When OR Scheduler declines the request revision initiated by office scheduler
      | TIME_NOT_AVAILABLE | MISSING_INFORMATION | OTHER | MESSAGE |
      | V                  |                    |       | Declined because of unavailable time |
    Then Alerts have NEEDS REVISION
      | Time not available | Declined because of unavailable time |
    And Browser quits
